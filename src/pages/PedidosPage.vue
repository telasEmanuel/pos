...
const confirmarPago = async (data: { montoPagado: number; comentarios: string; metodoPago: string; pagoDetalle: PaymentBreakdown; requiereFactura: boolean }) => {
  ...
  try {
    ...
    if (data.metodoPago === 'MIXTO' && data.pagoDetalle) {
      const desglosePago = `[Detalle Pago: Pesos: $${data.pagoDetalle.efectivo} USD: ${data.pagoDetalle.dolares} (Tasa: ${data.pagoDetalle.tasaCambio}) Tarjeta: $${data.pagoDetalle.tarjeta} Transf: $${data.pagoDetalle.transferencia}]`;
        ...
        <div v-else class="pedidos-grid">
          <div v-for="pedido in pedidosPendientes()" :key="pedido.id" class="pedido-card pending">
            ...
            <div class="pedido-actions">
              ...
              <button v-if="datos?.rol !== 'caja'" @click="editPedido(pedido)" class="btn-edit">
                ✎ Editar
              </button>
